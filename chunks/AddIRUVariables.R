...
D1 <- Data
D1$VNS <- zoo::na.approx (as.vector(D1$VNS), maxgap=1000, na.rm=FALSE, rule=2)
D1$VEW <- zoo::na.approx (as.vector(D1$VEW), maxgap=1000, na.rm=FALSE, rule=2)
D1$VSPD <- zoo::na.approx (as.vector(D1$VSPD), maxgap=1000, na.rm=FALSE, rule=2)
...
AA[,3] <- AA[,3] - G
# Sometimes accelerations are present but rotations are not. Don't replace existing accelerations.
if (!('BLONGA' %in% names(Data))) {
  Data$BLONGA <- AA[, 1]
  Data$BLATA  <- AA[, 2]
  Data$BNORMA <- AA[, 3]
}
if (!('BYAWR' %in% names(Data))) {
  
  D1$PITCH <- zoo::na.approx (as.vector(D1$PITCH), maxgap=1000, na.rm=FALSE, rule=2)
  D1$ROLL <- zoo::na.approx (as.vector(D1$ROLL), maxgap=1000, na.rm=FALSE, rule=2)
